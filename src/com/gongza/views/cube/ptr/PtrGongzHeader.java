...
import com.gongza.views.cube.indicator.PtrIndicator;

public class PtrGongzHeader extends FrameLayout implements PtrUIHandler {
	...
	public PtrGongzHeader(Context context) {
		super(context);
		initViews(null);
	}

	public PtrGongzHeader(Context context, AttributeSet attrs) {
		super(context, attrs);
		initViews(attrs);
	}

	public PtrGongzHeader(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void initViews(AttributeSet attrs) {
		...
		View header = LayoutInflater.from(getContext()).inflate(
				R.layout.cube_ptr_gongz_header, this);

		mRotateView = (ImageView) header
	...
	private void hideRotateView() {
		...
		// mRotateView.getDrawable().setLevel(20);
	...
	public void onUIPositionChange(PtrFrameLayout frame, boolean isUnderTouch,
		...
		if (mRotateView != null) {
			if (currentPos<=90) {
				tIndex=90;
			}else if (currentPos>=290) {
				tIndex=290;
			}else {
				tIndex=currentPos;
			}
			mRotateView.getDrawable().setLevel(tIndex);
		}
